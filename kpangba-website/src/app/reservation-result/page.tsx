        ...
        <div className='flex min-h-screen flex-col'>
            ...
            <div className='flex px-16 mt-2 gap-24 items-start text-start justify-start'>
                <div className='first flex flex-col gap-4 w-[9.6rem] '>
                    ...
                    <div>
                        ...
                        <div className='mt-2 flex gap-4'>
                            <input type="radio" />
                            <label
                                className="text-base font-normal text-[#302929]">
                                Buffet
                            </label>
                        </div>
                ...
                <div className='flex flex-col gap-4 w-[56.7rem] '>
                    <span className='bg-[#FEF8D2] mt-5 mb-2 items-center text-center justify-center rounded-lg border border-[#D0B61B] w-[21rem] py-3'>
                        12 Kmt-5pangba food on wheels in Lagos
                    </span>
                    <div className='border'></div>
                    <div className='w-[55rem] rounded-xl shadow-lg px-8 py-12 flex flex-col gap-8 bg-white'>
                        {locationResult.map((item, index) => (
                            <ResultCard key={index} text1={item.name} text2={item.subname} icon={<Star1 variant='Bold' color='#D0B61C' size="18" />} img={RestaurantPic} time={item.time} amount={item.amount} icon2={<Clock variant='Linear' color='#302929' size="22" />} number={item.number} />
                        ))}
                    </div>
                </div>
            </div>
