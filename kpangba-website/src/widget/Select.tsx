...
interface SelectProps {
    ...
    width?: string;
    options: OptionProps[];
    onChange?: React.Dispatch<React.SetStateAction<OptionProps | null>>;
    grouptext?: string;
        ...
        <div className="relative">
            <NewSelect
                ...
                placeholder={
                    label ? (
                        <p
                            className={`flex gap-3 items-center text-[#302929] ${textSmall ? "text-md" : "text-md"
                ...
                getOptionLabel={
                    (e: OptionProps) => {
                        return <p className={`flex items-center ${hasIconPadding ? "gap-2" : ""} ${textSmall ? "text-sm" : "text-md"} ${!textWhite ? 'text-blue-600' : 'text-white'}`}>
                            {hideOptionIcons ? e.icon || <Location size="32" color="#FF8A65" variant="Bold" /> : <></>}{e.label}</p>
                    }
                }
